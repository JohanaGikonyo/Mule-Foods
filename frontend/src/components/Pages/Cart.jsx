    ...
    try {
      if (totalQuantity !== 0) {
        ...
        const response = await axios.post(
          "https://mule-foods.onrender.com/orderapi/order",
          {
            location,
            name,
            totalCost,
      ...
      <div className="shadow-lg bg-white overflow-hidden sm:rounded-lg w-full">
        <table className="min-w-full divide-y divide-gray-200 border border-gray-300">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-1 py-2 text-xs font-medium text-gray-500 uppercase tracking-wider md:px-6 md:py-3 border border-gray-300">
                Product
              </th>
              <th className="px-1 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider md:px-6 md:py-3 border border-gray-300">
                Quantity
              </th>
              <th className="px-1 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider md:px-6 md:py-3 border border-gray-300">
                Cost (Kshs.)
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {items.map((item) => (
              <tr key={item.itemName}>
                ...
                <td className="px-1 py-2 md:px-6 md:py-3 whitespace-nowrap border border-gray-300">
                  {item.itemQuantity}
                </td>
                <td className="px-1 py-2 md:px-6 md:py-3 whitespace-nowrap border border-gray-300">
                  Kshs.{item.itemCost}
      ...
      <div className="flex items-center justify-around gap-3 mt-10">
        <div className="mt-10 mb-10">
          <button className="px-2 py-1 border border-orange-400 rounded-md">
            <NavLink to="/mainpage/home" className="flex flex-row">
          ...
          {circularProgress ? (
            ...
            <div className="mt-10 mb-10">
              <button className="px-1 py-1  rounded-md">
                <button
                  className="flex flex-row bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-md  items-center gap-2"
