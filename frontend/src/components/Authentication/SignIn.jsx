...
function SignIn() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "https://mule-foods.onrender.com/api/signin",
        {
          name,
          email,
  ...
  return (
    <div>
      ...
      <form
        className="flex flex-col justify-around items-center gap-4 p-2 rounded-md text-slate-700"
        ...
        <div className="flex flex-col justify-around items-center gap-4 bg-slate-50 p-2 m-10 rounded-md text-slate-700">
          ...
          >
            ...
            <TextField
              id="outlined-basic"
              type="text"
              label="Location around upperHill"
              variant="standard"
              onChange={(e) => setLocationAround(e.target.value)}
              required
            />
            <TextField
              id="outlined-basic"
          ...
          <div>
            {circularProgress ? (
              ...
              <Button type="submit" className="border border-orange-400 p-1 px-3 rounded-md flex items-center gap-1">
                Create Account
              </Button>
            )}
          </div>
          <div>
            <NavLink to="/login" className="text-blue-500 text-sm underline">
